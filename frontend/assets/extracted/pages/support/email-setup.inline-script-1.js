...
window.addEventListener("DOMContentLoaded", () => {
    ...
    const actionTypeEl = document.getElementById("email_action_type");
    const actionHintEl = document.getElementById("emailActionHint");
    const sendBtn = document.getElementById("sendEmailActionBtn");
    const actionToEmailEl = document.getElementById("action_to_email");
    const actionInvoiceIdEl = document.getElementById("action_invoice_id");

    function updateActionHint(){
        const actionType = String(actionTypeEl && actionTypeEl.value || "").trim();
        if(!actionHintEl) return;
        if(actionType === "forgot_password"){
            actionHintEl.textContent = "Use login page Forgot Password for end-user flow. This action sends forgot password email to To Email.";
            return;
        }
        if(actionType === "support_technician"){
            actionHintEl.textContent = "Support Technician sends payment detail attachment. Invoice ID is required.";
            return;
        }
        if(actionType === "vendor"){
            actionHintEl.textContent = "Vendor sends stock report attachment for products with stock count <= 1.";
            return;
        }
        actionHintEl.textContent = "Quotation and Invoice sending is already handled from Invoice page.";
    }

    if(actionTypeEl){
        actionTypeEl.addEventListener("change", updateActionHint);
        updateActionHint();
    }

    if(sendBtn){
        sendBtn.addEventListener("click", async () => {
            const action_type = String(actionTypeEl && actionTypeEl.value || "").trim();
            const to_email = String(actionToEmailEl && actionToEmailEl.value || "").trim();
            const invoice_id = Number(actionInvoiceIdEl && actionInvoiceIdEl.value || 0);
            ...
            if(action_type === "quotation" || action_type === "invoice"){
                alert("Quotation/Invoice email sending is already available in Invoice page.");
                return;
            }
            if(action_type === "support_technician" && (!Number.isFinite(invoice_id) || invoice_id <= 0)){
                alert("Invoice ID is required for Support Technician action.");
                return;
            }

            try{
                await request("/email-setup/send-action", "POST", {
                    action_type,
                    to_email,
                    invoice_id: Number.isFinite(invoice_id) && invoice_id > 0 ? invoice_id : null
                });
                showMessageBox("Email action sent");
            }catch(err){
                alert(err.message || "Failed to send email action.");
            }
        });
    }
});
